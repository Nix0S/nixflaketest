...
let
  cfg = config.elements.services.samba;
  ...
  shares-submodule = with types; submodule ({ name, ... }: {
    options = {
      ...
      only-owner-editable = mkBoolOpt false "Whether the share is only writable by the system owner (elements.user.name).";
...
in
{
  options.elements.services.samba = with types; {
    ...
    services.samba = {
      ...
      shares = mapAttrs
        (name: value: {
          ...
          "write list" = config.elements.user.name;
