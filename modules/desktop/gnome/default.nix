...
with lib.internal;
let
  cfg = config.elements.desktop.gnome;
  gdmHome = config.users.users.gdm.home;
in
{
  options.elements.desktop.gnome = with types; {
    enable =
      mkBoolOpt false "Whether or not to use Gnome as the desktop environment.";
    wallpaper = {
      light = mkOpt (oneOf [ str package ]) pkgs.elements.wallpapers.nix-wallpaper-nineish "The light wallpaper to use.";
      dark = mkOpt (oneOf [ str package ]) pkgs.elements.wallpapers.nix-wallpaper-nineish-dark-gray "The dark wallpaper to use.";
    };
  ...
  config = mkIf cfg.enable {
    elements.system.xkb.enable = true;
    elements.desktop.addons = {
      gtk = enabled;
      wallpapers = enabled;
    ...
    environment.systemPackages = with pkgs; [
      (hiPrio elements.xdg-open-with-portal)
      wl-clipboard
      gnome.gnome-tweaks
