...
with lib.internal;
let
  cfg = config.elements.apps.firefox;
  defaultSettings = {
...
in
{
  options.elements.apps.firefox = with types; {
  ...
  config = mkIf cfg.enable {
    elements.desktop.addons.firefox-nordic-theme = enabled;

    services.gnome.gnome-browser-connector.enable = config.elements.desktop.gnome.enable;
    ...
    elements.home.extraOptions = {
      programs.firefox = {
        ...
        package = pkgs.firefox-wayland.override (
          ...
          lib.optionalAttrs config.elements.desktop.gnome.enable {
            cfg = {
        ...
        profiles.${config.elements.user.name} = {
          inherit (cfg) extraConfig userChrome settings;
          id = 0;
          name = config.elements.user.name;
        };
      };
    };
  };
